...
func TestSpecIconThroughExec(t *testing.T) {
	...
	t.Run("无 icon 得默认", func(t *testing.T) {
		o, err := InitExecOpener(Spec{Name: "code", Cmd: "code"}, nil)
		if err != nil {
			t.Fatalf("构造失败: %v", err)
		}
...
func TestTitleDefault(t *testing.T) {
	o1, _ := InitExecOpener(Spec{Name: "code", Cmd: "code"}, nil)
	if got := o1.Title(); got != "用 code 打开" {
		t.Fatalf("缺省 title 应由 name 生成, got %q", got)
	}
	o2, _ := InitExecOpener(Spec{Name: "finder", Title: "打开所在目录", Cmd: "open $0"}, nil)
