...
func newMdCmd(a *app.App) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runMd(args[0], port)
		},
	}
	cmd.Flags().IntVarP(&port, "port", "p", server.DefaultPort, "server port")
	return cmd
}

func runMd(rawPath string, port int) error {
	...
	if st, _ := serve.Status(port); !st.Running {
		return fmt.Errorf("server 未运行（port=%d），请先执行: cube server start", port)
	}

	pageURL := fmt.Sprintf("http://localhost:%d/md?path=%s", port, url.QueryEscape(absPath))
