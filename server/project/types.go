...
type SettingsSpec struct {
	Scan  []ScanRuleSpec  `json:"scan"`
	Clone []CloneRuleSpec `json:"clone"`
}

// ScanRuleSpec 扫描规则的存储形态（用户所写路径，可含 ~/）。
...
// CloneRuleSpec clone 规则的存储形态（localPath 可含 ~/）。
type CloneRuleSpec struct {
...
// makeScanRules 存储形态转领域形态：展开 ~/ 为绝对路径，校验目录存在
// （不存在的规则降级跳过、打日志，不阻断其它规则）。
func makeScanRules(specs []ScanRuleSpec) []ScanRule {
	var rules []ScanRule
	for _, r := range specs {
...
// makeCloneRules 存储形态转领域形态：localPath 展开 ~/ 为绝对路径
// （不校验存在——clone 时会自动创建）；相对路径是配置错误，跳过。
func makeCloneRules(specs []CloneRuleSpec) []CloneRule {
	var rules []CloneRule
	for _, r := range specs {
