...
- [`1044-forge-account-only/`](proposals/1044-forge-account-only/) — 推翻 namespace 模型：各平台「按命名空间列仓库」端点分裂且不可靠（github/gitee 只回公开库、gitee org 列表混幽灵条目），只保留 account（`GET /user/repos` 认证端点「列表即真相」）；Namespace/detect/forgeNamespaces 节删除，拉取挂 account，孤儿 = 同 namespace（URL 推导）但不在拉取结果
- [`1042-forge页/`](proposals/1042-forge页/) — 前台 forge 页：以 forge 上仓库为入口的列表视图（类项目列表页、无 tree 模式），展示对账状态、未 clone 复制 clone 命令（不做 clone 执行）；拉取结果落盘 + fetchedAt 前端展示
