...
> **状态**：✅ 已实现（260817 归档）
>
> 落地与提案一致（后端零渲染，只读原文 API）；并超出基线：目录浏览模式
> （/api/md/list + 前端文件树）、md 主题切换、侧栏可拖宽度。打开能力做于
> Opener 域（POST /api/opener/open，role 按路径类型校验），项目打开也归一到此端点。
> **依赖**：
> - [`1001-server后台常驻与HTTP管理`](1001-server后台常驻与HTTP管理/)（已落地；原「按需启动」提案实施时改为显式 start，`cube md` 需处理「server 未跑」场景）
> - [`1002-前端栈迁移`](1002-前端栈迁移/)（渲染页面归属前端工程）
...
- cube 已规划前端栈迁移（`1002-前端栈迁移`，Vite + React + TypeScript），未来所有 Web 页面都走前端工程
...
```
1001-server后台常驻与HTTP管理   (无依赖)
        ↓
1002-前端栈迁移        (无强依赖，可与上一个并行)
        ↓
1005-md渲染            (依赖前两者都落地)
```
