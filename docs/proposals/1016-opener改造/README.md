# opener 改造：icon + web 形态 + 落库与 Web 配置
...
3. **config.json 配置方式到顶了**：需要 Web 配置界面做增删改；顺理成章地把 opener 数据从 config.json 迁到 sqlite 表，**每次直接读表、不做内存缓存**（openers 数量个位数，本地 sqlite 毫秒级；Web 改完立刻生效，CLI/Web 永远一致，省掉缓存失效逻辑）。
...
- gorm model `opener.Opener`（name 唯一索引、cmd/roles/icon 存 JSON 文本列），AutoMigrate 就近放 opener service（与 history 做法一致）。
- **JSON 校验收敛在 Go 类型读写边界**：字段用自定义 Scanner/Serializer，读时 unmarshal 进领域类型（parseRoles / icon 解析，失败按现有「单条跳过不阻断」降级）；写时只接受已构造好的领域类型——坏数据唯一入口是 save API，service 写库前先走领域构造函数校验，失败 400 中文错误，落不了库。
- 一次性迁移：启动时表空且 config.json 有 openers 节 → 导入后**删除 config.json 的 openers 节**并记日志，避免双事实源。
- Service 的 `AllOpeners()`/`FindByName()` 等每次查库。
...
1. **表 + 迁移 + Service 改读库**：行为等价重构，`opener_test.go` 大部分平移（config 构造换成库构造）。
2. **Opener 接口化**：exec/web 两实现，cmd 层与 handler 无感。
3. **icon 字段**：领域类型 + 落库 + 提取 util（darwin）。
4. **Web CRUD API + 前端配置页**。
5. **收尾**：干掉前端 finder/stree 硬编码；更新 `docs/spec/现状.md` 3.3/3.5/6.2 与 web API 表。

## 验收标准

1. config.json 的 openers 自动迁移进表，迁移后 config.json 无 openers 节；既有 `open`/`open-path`/`diff`/`alfred opener-search` 行为不退化。
...
5. 坏配置（未知 role、占位符越界、畸形 JSON）经 save API 返回中文错误，不落库；库内历史脏数据单条跳过不阻断启动。
