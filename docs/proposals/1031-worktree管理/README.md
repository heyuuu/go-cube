...
> **状态**：📝 待评审（核心决策已讨论定稿，见「讨论定稿」）
>
> **关联**：[`1016-opener改造`](../archived/1016-opener改造/README.md)（打开 worktree 复用 RoleOpenDir，本提案不改 opener）；[`1011-工作台git树面板`](../archived/1011-工作台git树面板/README.md)（worktree 作为 TreeSource 的读侧已就绪）；[`1029-scan-clone规则迁移settings`](../1029-scan-clone规则迁移settings/README.md)（scanRule maxDepth 与目录位置决策相关）；[`1032-worktree归并为项目打开目标`](../1032-worktree归并为项目打开目标/README.md)（**口径依赖，待评审联动**：1032 若先行落地，worktree 不再是独立项目——本提案验收 1 的「项目列表」口径变为主项目的打开目标，目录位置决策的 maxDepth 论据随之失效（worktree 可放任意位置）；写侧能力与「写后定向刷新 gitcache」不受影响且更加必要）。
...
1. **worktree 目录位置：repo 同级的扁平命名 `<repoName>--<branch>`**，不放 repo 内。理由：
   - 同级同深度：只要 repo 本身在该 scanRule 的 maxDepth 内，worktree 必然也在——`<repo>.worktrees/<branch>/` 这类嵌套方案比仓库深一级，仓库贴着 maxDepth 时 worktree 会被扫描卡掉；
   - 扁平不引入 repo 内嵌套扫描歧义；
   - 与既有个人习惯一致。
   Web UI 新建对话框**预填**该默认路径、允许手改；不做配置化模板（真有需求再另加，届时挂全局 config，只记模式不记实例路径）。
...
4. 写操作成功后失效/刷新 gitcache 对应路径，新 worktree 的出现/消失即时可见，不等 TTL 自愈。
5. **定位修订（伴随）**：出口优先级从「CLI 优先 + 本地 Web」调整为「**Web UI 优先，CLI 简单可行兜底**」。CLI 的目标是 web 不可用时兜底不至于寸步难行，便利性操作以 Web UI 为主。本需求的 CLI 兜底方案就是直接用 git 命令，因此 **CLI 子命令本期不做**（将来作为便利性补充可加）。AGENTS.md 与 `docs/spec/现状.md` 的定位表述随本提案落地一并修订。
...
- `WorktreeAdd(path, opts)`：推导预填路径（repo 父目录 + `<repoName>--<branch>`）、目标目录存在性校验（git 要求不存在或为空）、预检被检出分支冲突 → 调 git → 刷新 gitcache → 返回新 worktree 信息（供 UI 直接发起 open）。
...
gitcache 刷新落点实现期定（候选：gitcache 增加包级失效/定向刷新入口，避免 workbench 对 project 域的反向依赖过深）。
...
- refs 列表加分支删除入口；
- 删除当前 workbench 正在查看的 worktree 后，前端导航回主副本或项目页。
...
1. `util/git` 写侧函数 + 单测（testfixture 真仓库，覆盖三种 add 形态 / 被检出分支拒绝 / force 与非 force 删除）；
...
5. 定位修订（AGENTS.md「项目简介」/ 现状.md 定位段）+ 现状.md API 清单同步，验收后归档提案。

## 验收标准

1. 从任一 commit/分支可新建 worktree（新建分支 / 检出已有分支 / detached 三种形态），默认路径 `<repo>--<branch>`，新副本即时出现在项目列表（worktree tag，无需等 TTL）；
...
6. AGENTS.md 与现状.md 的定位表述及 API 清单已同步。
